...
import { doc, setDoc, getDoc, deleteDoc, serverTimestamp, type Firestore } from 'firebase/firestore';
import { auth as _auth, db as _db } from '../firebase';
import { firebaseHandler, firebaseVoidHandler, type ApiResponse } from '../handler';

// auth/db may be null when Firebase is not configured.
// These are safe non-null casts: all calls live inside firebaseHandler
// which requires Firebase to be available, and the AuthContext already
// checks isFirebaseAvailable() before calling these methods.
const auth = _auth as import('firebase/auth').Auth;
